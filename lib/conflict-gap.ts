 ...
 *
 * This module is arithmetic only — no policy, no I/O, no imports. It is a
 * BYTE-IDENTICAL copy of `src/utils/conflict-gap.ts` in sidelineiq-agents,
 * apart from this paragraph, and both are pinned by
 * `tests/fixtures/conflict-gap-cases.json`, which exists in both repos. If you
 * change the arithmetic, change it there first, bump
 * CONFLICT_GAP_HELPER_VERSION, re-record the fixture and copy both files
 * across together. Do not add a seventh formula anywhere.
 */
